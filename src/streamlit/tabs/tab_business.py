"""
Business Analytics & Strategy Recommendations Tab

Provides market price visualization and AI-generated operational recommendations
based on electricity prices, weather forecasts, and system conditions.
"""
...
def _get_top_recommendations(df_market: pd.DataFrame, df_weather: pd.DataFrame) -> list:
    ...
    - Negative prices → Charge from grid
    - High prices (>300 EUR/MWh) → Discharge to grid
    - High solar + low prices → Store PV energy
    - High temperatures (>30°C) → Thermal management
    - High price volatility → Intraday arbitrage

    Args:
        df_market: Market prices dataframe
        df_weather: Weather analytics dataframe

    Returns:
        list: Top 2 recommended operational strategies
    """
